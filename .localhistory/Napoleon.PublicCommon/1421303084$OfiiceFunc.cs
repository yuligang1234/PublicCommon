...
namespace Napoleon.PublicCommon
{
    public static class OfiiceFunc
    {
        ...
        public static string Excel(this DataTable dt, string topTitle, string[] hiddenColumns, string[] titles, bool isId, string footer)
        {
            int i = 0;//序号
            ...
            if (!string.IsNullOrWhiteSpace(topTitle))
            {
                ...
                sbHtml.Append("<tr>");
                //标题（只能是一级标题）
                sbHtml.AppendFormat("<td colspan='{0}' style='font-size: 14px;text-align:center;background-color: #DCE0E2; font-weight:bold;' height='25'>{1}</td>", topTitle);
                sbHtml.Append("</tr>");
            }
            sbHtml.Append("<tr>");
            //标题（只能是一级标题）
