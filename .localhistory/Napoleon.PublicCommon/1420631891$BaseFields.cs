...
namespace Napoleon.PublicCommon
{
    public static class BaseFields
    {
        ...
        public static void WriteCookie<T>(string strname, T t, int days)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strname];
            if (cookie == null)
                cookie = new HttpCookie(strname);
        ...
        public static Account ReadCookieAsObj(string strname)
        {
            if (HttpContext.Current.Request.Cookies[strname] != null)
            {
                return DSerializeToObject<Account>(UrlDecode(HttpContext.Current.Request.Cookies[strname].Value));
            }
            return null;
        }
