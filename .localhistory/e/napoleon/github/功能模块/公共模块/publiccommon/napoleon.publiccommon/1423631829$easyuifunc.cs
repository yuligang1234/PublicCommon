        ...
        public static string ConvertTableToGridJson(this DataTable dt, int total)
        {
            ...
            if (dt.Rows.Count > 0)//如果不判断的话，在dt没有数据的情况下，会出现格式错误{[]}
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    {
                        json.AppendFormat("\"{0}\":\"{1}\",", column.ColumnName, row[column].ToString().Replace(BaseFields.SymbolNewLine, BaseFields.SymbolBlank).Replace(BaseFields.SymbolDoubleQuotes, BaseFields.SymbolSigleQuotes));//去掉换行符
                    }
